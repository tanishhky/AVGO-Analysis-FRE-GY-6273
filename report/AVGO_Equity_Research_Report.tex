...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\raisebox{-0.15\height}{\includegraphics[height=12pt]{logo_small.png}} \hspace{2pt} \color{nyuviolet}\bfseries\small Broadcom Inc. (AVGO) \textbar{} Equity Research}
\fancyhead[R]{\color{nyugrey}\small 29 April 2026 \textbar{} \textbf{BUY} \textbar{} PT \$475}
...
% --- Top white band with firm logos only ---
...
\node[anchor=north west,inner sep=0pt]
  ...
  {\includegraphics[height=0.52in]{logo_small.png}\hspace{8pt}\includegraphics[height=0.52in]{logo_big.png}};

% --- Violet accent stripe ---
\fill[nyuviolet] ([yshift=-0.85in]current page.north west) rectangle ([yshift=-0.90in]current page.north east);

% --- Black band with all stock info ---
\fill[tickerblack] ([yshift=-0.90in]current page.north west) rectangle ([yshift=-1.95in]current page.north east);

% Left side: Broadcom logo + date + sector + industry
\node[anchor=north west,inner sep=0pt]
  at ([xshift=0.6in,yshift=-0.98in]current page.north west)
  {\includegraphics[height=0.55in]{broadcom_logo.png}};
% \node[anchor=north west,text=white!80,font=\fontsize{9}{11}\sffamily,inner sep=0pt]
%   at ([xshift=0.6in,yshift=-1.58in]current page.north west) {29 April 2026};
\node[anchor=north west,text=white!60,font=\fontsize{12}{15}\sffamily,inner sep=0pt]
  at ([xshift=0.6in,yshift=-1.72in]current page.north west) {Sector: Technology \textbar{} Semiconductors \& Software};
% \node[anchor=north west,text=white!50,font=\fontsize{8}{10}\itshape,inner sep=0pt]
%   at ([xshift=0.6in,yshift=-1.84in]current page.north west) {NYU MSFE Equity Research \textbar{} Initiation of Coverage};

% Right side: AVGO ticker, exchange, price, returns (stacked)
\node[anchor=north east,text=white,font=\fontsize{30}{36}\bfseries,inner sep=0pt]
  at ([xshift=-0.6in,yshift=-0.98in]current page.north east) {AVGO};
\node[anchor=north east,text=white!70,font=\fontsize{10}{12}\sffamily,inner sep=0pt]
  at ([xshift=-0.6in,yshift=-1.35in]current page.north east) {NASDAQ \textbar{} BROADCOM INC \textbar{} USD};
\node[anchor=north east,text=white,font=\fontsize{22}{28}\bfseries,inner sep=0pt]
  at ([xshift=-0.6in,yshift=-1.50in]current page.north east) {\$399.83};
\node[anchor=north east,text=accentgreen!70!white,font=\fontsize{10}{13}\bfseries\sffamily,inner sep=0pt]
  at ([xshift=-0.6in,yshift=-1.80in]current page.north east) {\(\blacktriangle\) +36.3\% (30D) \quad +107.7\% (12M)};
...
\node[anchor=north west,text=tickerblack,font=\fontsize{24}{30}\bfseries,inner sep=0pt,text width=7.2in]
  ...
  {The \$2T AI Compounder: Why Broadcom's Moat Is Wider Than You Think};
\node[anchor=north west,text=nyuviolet,font=\fontsize{14}{18}\bfseries,inner sep=0pt,text width=7.2in]
  ...
  {Initiating BUY \textbar{} 12-month PT \$475 \textbar{} +19\% upside};
...
\node[anchor=west,text=accentgreen,font=\fontsize{12}{14}\bfseries]
  at ([xshift=2.85in,yshift=-4.23in]current page.north west) {Implied upside +18.8\%};

\node[anchor=north west,inner sep=0pt]
  at ([xshift=0.6in,yshift=-4.80in]current page.north west) {%
\includegraphics[width=3.55in]{fig1_price_chart.png}};

\node[anchor=north east,inner sep=0pt]
  at ([xshift=-0.55in,yshift=-4.80in]current page.north east) {%
\includegraphics[width=3.55in]{fig4_football_field.png}};
% =============================================================================
% Tables side by side (3 columns below charts)
% =============================================================================
% Left table - Market Data
\node[anchor=north west,inner sep=0pt,text width=2.4in]
  at ([xshift=0.6in,yshift=-6.90in]current page.north west) {%
...
Last price (29-Apr-26) & \$399.83 \\
52W range & \$191 -- \$423 \\
Market cap / EV & \$1.99T / \$2.04T \\
...
% Middle table - Valuation Triangulation
\node[anchor=north,inner sep=0pt,text width=2.4in]
  at ([yshift=-6.90in]current page.north) {%
\renewcommand{\arraystretch}{1.0}\setlength{\tabcolsep}{3pt}%
{\color{nyuviolet}\rule{\linewidth}{0.5pt}}\\[-2pt]
...
Comps Fwd P/E (22$\times$) & \$414 \\
Comps EV/EBITDA (18$\times$) & \$409 \\
Monte Carlo median & \$456 \\
Street median (n=11) & \$460 \\
\rowcolor{rowshade}\color{nyuviolet}\textbf{Our 12M PT*} & \textbf{\$475} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\vspace{1pt}\tiny *Anchored on MC mean.} \\
\end{tabularx}}
};

% Right table - Key Catalysts
\node[anchor=north east,inner sep=0pt,text width=2.4in]
  at ([xshift=-0.55in,yshift=-6.90in]current page.north east) {%
\renewcommand{\arraystretch}{1.0}\setlength{\tabcolsep}{3pt}%
{\color{nyuviolet}\rule{\linewidth}{0.5pt}}\\[-2pt]
...
{\scriptsize\color{nyugrey}\textit{Student-prepared educational research; not investment advice. Data as of 29 April 2026.}}
};
...
\textbf{Recommendation: BUY \textbar{} 12-Month PT: \$475 (18.8\% upside)}

\begin{minipage}[t]{0.55\textwidth}
We initiate coverage of Broadcom (AVGO) with a \textbf{BUY} rating and a December-2026 price target of \$475, representing 18.8\% upside to the 29-April spot price of \$399.83. Our valuation triangulates a \$401--\$474 fair-value range across DCF, sum-of-the-parts, peer comparables, and a 10{,}000-path Monte Carlo simulation that places the median intrinsic price at \$456 with a 68.3\% probability of trading above spot at current levels.
...
We are constructive on \textbf{valuation after the recent pullback}: shares have retreated from the April highs but our Monte Carlo's $E[\text{intrinsic}] = \$471$ still exceeds spot by 18\%, and the right tail is fat (P75 \$544, P95 \$711) reflecting realistic upside if FY27 AI revenue prints at the high end of the analyst range (\$130--150B per UBS / Barclays).

Our PT of \$475 sits above the Street median (\$460) and reflects our view that the bear-case PTs from D.A.\ Davidson (\$335) and Cantor Fitzgerald (\$300) are unlikely to materialize given the catalyst path. We see further upside if the December 2026 FY27 guide validates the \$120B+ AI revenue scale.
\end{minipage}\hfill
...
\item \textbf{Recommendation derived from analysis.} Six valuation methods converge: MC median \$456, DCF \$436, Comps P/E \$414, Comps EV/EBITDA \$409, SOTP \$401, Street median \$460. Our \$475 PT sits modestly above the Street median, reflecting the catalyst path through year-end and the right-tail in our Monte Carlo distribution.
\end{enumerate}
...
\begin{tabular}{p{5.6cm}p{3.0cm}p{2.5cm}p{3.5cm}}
\arrayrulecolor{nyuviolet}\toprule
...
\textbf{Median (n=11)} & \textbf{9 Buy/OW} & \textbf{460} & -- \\
\arrayrulecolor{nyuviolet}\bottomrule
\end{tabular}
...
The mean PT of the eleven covering firms in our dataset is \$449; the median is \$460. Nine of eleven sit above the current spot price.

\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{fig12_analyst_pts.png}
\caption{Sell-side price-target distribution. Median \$460; 9 of 11 firms above spot. Source: Benzinga, individual broker reports.}
\end{figure}
...
We project FY26E revenue of \$105 billion (+64\% YoY) and FY30E revenue of \$292 billion, with AI semi growing from \$60 billion in FY26 to \$235 billion by FY30 (a 41\% five-year CAGR). Our deterministic FY27 AI revenue assumption of \$115 billion sits at the conservative end of the analyst range (\$100--150 billion) and within management's stated ``more than \$100 billion'' guidance. Note that our Monte Carlo simulation (Section 4.4) recalibrates the FY27 AI mean to \$125B---the Street consensus midpoint---to capture the full distribution of plausible outcomes.
...
\textbf{Result.} The base-case DCF implies a price of \$436 per share (9.0\% upside), with the WACC$\times$g sensitivity producing a \$402--\$474 range at $\pm$50bps WACC and base terminal growth.
...
Segment EBITDA margins are derived as follows: software at 77\% reflects a high-margin steady state consistent with Broadcom's curated top-600-enterprise-customer subscription model and peer benchmarks (Oracle 50--55\%, Adobe 45--50\%, Microsoft software segment 65--75\%, with 77\% the upper bound for Broadcom's pricing-power-driven model). Semi at 62\% reverse-engineers from the FY25A blended 67.7\% EBITDA margin and the segment revenue mix.

\subsection{Trading comparables}

We benchmark Broadcom against seven semiconductor peers (NVDA, AMD, MRVL, QCOM, TXN, INTC, TSM). Peer median forward P/E is 28.3$\times$; median EV/EBITDA is 31.8$\times$ TTM (skewed by AMD/MRVL). Applying conservative multiples to our \$18.83 CY27E EPS estimate (vs.\ analyst mean of \$19.85):
...
\midrule
Forward P/E (on CY27E EPS \$18.83)        & 18$\times$ \$339  & 22$\times$ \$414 & 26$\times$ \$490 \\
...
Probability $>$ spot & \textbf{68.3\%} & 95th percentile & \$711 \\
\arrayrulecolor{nyuviolet}\bottomrule
\end{tabular}
...
The 68.3\% probability that intrinsic value exceeds spot, combined with $E[\text{intrinsic}] = \$471$ vs.\ spot of \$399.83, supports our BUY rating.
...
The methods cluster within a defensible \$400--\$510 range. Our \$475 price target anchors on the Monte Carlo expected value of \$471 (the probability-weighted intrinsic value across 10,000 simulations), rounded up by \$4 to reflect right-tail optionality from FY27 AI revenue printing above the \$125B Street midpoint. This level sits modestly above the Street median (\$460) and within the upper half of every method's range---a level consistent with FY27 AI revenue printing in the \$120--130B range, where management's ``\$100B+'' floor and the Street upper-bound converge.
...
After a 107.7\% twelve-month run, our DCF, SOTP, and comps cluster at \$400--440, but our Monte Carlo (calibrated to the Street consensus FY27 AI midpoint of \$125B) gives a 68.3\% probability of upside vs.\ spot with $E[\text{intrinsic}] = \$471$. We initiate at BUY with a \$475 December-2026 price target, equivalent to 19\% upside and modestly above the Street median (\$460). We would consider further upgrading our PT on a positive print to FY27 AI revenue scale at the December 2026 guide.
...
{\footnotesize\color{accentred}\textbf{Disclaimer.} This report is a student-prepared educational exercise authored by NYU MSFE students. It is not investment advice, not a recommendation to transact in AVGO securities, and not for distribution. Data as of 29 April 2026.}
